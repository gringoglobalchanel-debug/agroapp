                ...
                try {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        if (response.isSuccessful) {
                            Toast.makeText(this@DriverRegisterActivity, "Cuenta creada. Ahora inicia sesion.", Toast.LENGTH_LONG).show()
                            finish()
                        } else {
                            val error = response.errorBody()?.string() ?: "Error desconocido"
                            val msg = try { org.json.JSONObject(error).getString("error") } catch (e: Exception) { error }
