...
class AdminViewModel : ViewModel() {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    _message.value = "Producto creado"
                    loadProducts()
                } else { _message.value = "Error al crear" }
            } catch (e: Exception) { _message.value = "Error: ${e.message}" }
            _loading.value = false
        }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    loadProducts()
                } else { _message.value = "Error al actualizar" }
            } catch (e: Exception) { _message.value = "Error: ${e.message}" }
            _loading.value = false
        }
    }
    ...
    fun updateStock(productId: Int, quantity: Double, changeType: String, notes: String? = null) {
        viewModelScope.launch {
            _loading.value = true
            try {
                val response = api.updateProductStock(SessionManager.getToken(), productId, UpdateStockRequest(productId, quantity, changeType, notes))
                if (response.isSuccessful) {
                    _message.value = "Stock actualizado"
                    loadProducts()
                } else { _message.value = "Error al actualizar stock" }
            } catch (e: Exception) { _message.value = "Error: ${e.message}" }
            _loading.value = false
        }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    loadDriverPayments()
                } else { _message.value = "Error al procesar" }
            } catch (e: Exception) { _message.value = "Error: ${e.message}" }
            _loading.value = false
        }
    }
